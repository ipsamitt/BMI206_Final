...
profile_files <- list.files(path = "/Users/ipsamittra/Desktop/BMI_206", pattern = "*.profile", full.names = TRUE)
...
ggplot(accuracy_df, aes(x = Profile, y = Accuracy, fill = Profile)) +
 ...
 # coord_flip() +  # Flip the coordinates for better visibility
  ...
  ggtitle("Comparison of Prediction Accuracy across PLINK Profiles using BBJ GWAS") +
  theme_minimal() +
...
ggplot(merged_data, aes(x = Profile, y = SCORESUM, fill = Profile)) +
  geom_boxplot() +
  labs(x = "PLINK Profile", y = "Predicted Score (SCORE)") +
  ggtitle("Comparison of Predicted Scores across PLINK Profiles using BBJ GWAS") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
profile_subset <- merged_data %>% filter(Profile %in% c("hum_BBJ_AFR_plink.profile", "hum_BBJ_AMR_plink.profile"))

# Perform the t-test to compare the SCOREs between the two profiles
t_test_result <- t.test(SCORESUM ~ Profile, data = profile_subset)
...
anova_result <- aov(SCORESUM ~ Profile, data = merged_data)

# Print the ANOVA result
summary(anova_result)

# Perform pairwise t-tests (adjusted for multiple comparisons)
pairwise_t_test <- pairwise.t.test(merged_data$SCORESUM, merged_data$Profile, p.adjust.method = "bonferroni")
...
# Load your plink.profile file
plink_file <- "/Users/ipsamittra/Desktop/BMI_206/hum_BBJ_AFR_plink.profile"
plink_data <- fread(plink_file)

# Check the first few rows to identify the column with the scores
head(plink_data)

library(ggplot2)

# Plot the histogram of scores
ggplot(plink_data, aes(x = SCORESUM)) +
  geom_histogram(binwidth = 0.1, fill = "skyblue", color = "black", alpha = 0.7) +
  labs(title = "Distribution of Scores from PLINK Profile", x = "Score", y = "Frequency") +
  theme_minimal()
